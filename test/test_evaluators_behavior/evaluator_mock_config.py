...
def create_flow_side_effect(
    ...
    explanation: str = DEFAULT_EXPLANATION
) -> Callable[[int], Awaitable[str | Dict[str, Any]]]:
    """
    Factory function to create async side effect functions for mocking evaluator flows.
...
def get_flow_side_effect_for_evaluator(evaluator_name: str) -> Callable[[int], Awaitable[str | Dict[str, Any]]]:
    """
    ...
    if evaluator_name not in EVALUATOR_CONFIGS:
        raise ValueError(f"Evaluator '{evaluator_name}' not recognized. Available evaluators: {list(EVALUATOR_CONFIGS.keys())}")
