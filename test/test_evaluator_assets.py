...
EVALUATORS_DIR = Path(__file__).parent.parent / "assets" / "evaluators" / "builtin"
...
@pytest.mark.parametrize("evaluator_name,spec_file_path", get_all_evaluator_spec_files())
def test_validate_evaluators_schema(evaluator_name: str, spec_file_path: Path):
    """Test that evaluator spec.yaml files have valid initParameterSchema and dataMappingSchema.
    ...
    assert spec_data is not None, f"Failed to load spec.yaml for {evaluator_name}"
    
    # Validate initParameterSchema
    if "initParameterSchema" in spec_data:
        init_schema = spec_data["initParameterSchema"]
        try:
            # Create a validator instance and check if the schema itself is valid
            Draft202012Validator.check_schema(init_schema)
        except Exception as e:
            pytest.fail(f"initParameterSchema validation failed for {evaluator_name}: {str(e)}")
    ...
    if "dataMappingSchema" in spec_data:
        ...
        try:
            ...
            pytest.fail(f"dataMappingSchema validation failed for {evaluator_name}: {str(e)}")
    ...
    if "outputSchema" in spec_data:
        ...
        try:
            ...
            pytest.fail(f"outputSchema validation failed for {evaluator_name}: {str(e)}")
    
    # Ensure at least one of the schemas exists
    assert "initParameterSchema" in spec_data or "dataMappingSchema" in spec_data, \
        f"spec.yaml for {evaluator_name} must contain at least initParameterSchema or dataMappingSchema"
...
def test_all_evaluators_have_spec_files():
    ...
    assert len(missing_spec_files) == 0, \
        f"The following evaluators are missing spec.yaml files: {', '.join(missing_spec_files)}"
