...
ToolUseQualityEvaluator batches five evaluators into a single LLM call, so its output shape
...
from ...builtin.tool_use_quality.evaluator._tool_use_quality import ToolUseQualityEvaluator


def _make_real_evaluator() -> ToolUseQualityEvaluator:
    model_config = AzureOpenAIModelConfiguration(
        azure_endpoint=os.getenv("AZURE_OPENAI_ENDPOINT"),
        azure_deployment=os.getenv("AZURE_OPENAI_DEPLOYMENT"),
        api_version=os.getenv("AZURE_OPENAI_API_VERSION", "2024-08-01-preview"),
    )
    return ToolUseQualityEvaluator(model_config=model_config, credential=DefaultAzureCredential())
...
@pytest.mark.quality
class TestToolUseQualityEvaluatorQuality:
    """Quality tests for ToolUseQualityEvaluator with single-turn (query/response) input."""
