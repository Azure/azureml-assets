...
"""Quality tests for the Tool Use Quality Evaluators (single-turn) with real flow execution.

ToolUseQualityEvaluators batches five evaluators into a single LLM call, so its output shape
...
from ...builtin.tool_use_quality.evaluator._tool_use_quality import ToolUseQualityEvaluators


def _make_real_evaluator() -> ToolUseQualityEvaluators:
    model_config = AzureOpenAIModelConfiguration(
        azure_endpoint=os.getenv("AZURE_OPENAI_ENDPOINT"),
        azure_deployment=os.getenv("AZURE_OPENAI_DEPLOYMENT"),
        api_version=os.getenv("AZURE_OPENAI_API_VERSION", "2024-08-01-preview"),
    )
    return ToolUseQualityEvaluators(model_config=model_config, credential=DefaultAzureCredential())
...
@pytest.mark.quality
class TestToolUseQualityEvaluatorsQuality:
    """Quality tests for ToolUseQualityEvaluators with single-turn (query/response) input."""
