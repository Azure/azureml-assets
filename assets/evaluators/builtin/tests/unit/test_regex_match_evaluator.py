...
class TestRegexMatchEvaluatorDynamicPatterns:
    """Tests for dynamic patterns with column references.

    Dynamic patterns contain {{column_name}} references that are resolved
    at evaluation time from the eval_input kwargs.
    """

    def test_dynamic_pattern_with_column_reference_match(self):
        """Test dynamic pattern resolves column reference and matches."""
        evaluator = RegexMatchEvaluator(patterns=r"(?i)ANSWER\s*:\s*{{correct_answer}}")
        result = evaluator(response="ANSWER: B", correct_answer="B")
        assert result[REGEX_MATCH] == 1.0

    def test_dynamic_pattern_with_column_reference_no_match(self):
        """Test dynamic pattern resolves but doesn't match wrong answer."""
        evaluator = RegexMatchEvaluator(patterns=r"(?i)ANSWER\s*:\s*{{correct_answer}}")
        result = evaluator(response="ANSWER: A", correct_answer="B")
        assert result[REGEX_MATCH] == 0.0

    def test_dynamic_pattern_with_item_dot_notation(self):
        """Test dynamic pattern with item.column notation."""
        evaluator = RegexMatchEvaluator(patterns=r"(?i)ANSWER\s*:\s*{{item.correct_answer}}")
        result = evaluator(response="ANSWER: C", **{"item.correct_answer": "C"})
        assert result[REGEX_MATCH] == 1.0

    def test_multiple_dynamic_patterns(self):
        """Test multiple patterns with column references."""
        evaluator = RegexMatchEvaluator(
            patterns=[
                r"(?i)ANSWER\s*:\s*{{expected}}",
                r"(?i)The answer is {{expected}}",
            ]
        )
        result = evaluator(response="The answer is D", expected="D")
        assert result[REGEX_MATCH] == 1.0
        assert result[MATCHED_PATTERN_INDEX] == 1

    def test_dynamic_pattern_escapes_special_chars(self):
        """Test that column values with regex special chars are escaped."""
        evaluator = RegexMatchEvaluator(patterns=r"Result:\s*{{value}}")
        # The value contains regex special characters that should be escaped
        result = evaluator(response="Result: [x+y]", value="[x+y]")
        assert result[REGEX_MATCH] == 1.0

    def test_dynamic_pattern_multiple_references_in_one_pattern(self):
        """Test pattern with multiple column references."""
        evaluator = RegexMatchEvaluator(patterns=r"{{prefix}}:\s*{{answer}}")
        result = evaluator(response="ANSWER: B", prefix="ANSWER", answer="B")
        assert result[REGEX_MATCH] == 1.0

    def test_mixed_static_and_dynamic_content(self):
        """Test pattern with both static regex and dynamic references."""
        evaluator = RegexMatchEvaluator(patterns=r"(?i)Option\s+{{correct}}\s+is\s+(correct|right)")
        result = evaluator(response="Option A is correct.", correct="A")
        assert result[REGEX_MATCH] == 1.0

    def test_dynamic_pattern_case_insensitive(self):
        """Test dynamic pattern with case-insensitive flag."""
        evaluator = RegexMatchEvaluator(patterns=r"(?i){{expected}}")
        result = evaluator(response="answer: B", expected="B")
        assert result[REGEX_MATCH] == 1.0

    def test_dynamic_pattern_missing_column_raises_error(self):
        """Test that missing column reference raises KeyError."""
        evaluator = RegexMatchEvaluator(patterns=r"(?i)ANSWER:\s*{{missing_column}}")
        with pytest.raises(KeyError, match="missing_column"):
            evaluator(response="ANSWER: A")

    def test_dynamic_pattern_with_none_value(self):
        """Test dynamic pattern when column value is None."""
        evaluator = RegexMatchEvaluator(patterns=r"(?i)ANSWER:\s*{{value}}")
        # None becomes "None" string after str() conversion
        result = evaluator(response="ANSWER: None", value=None)
        assert result[REGEX_MATCH] == 1.0

    def test_dynamic_pattern_with_numeric_value(self):
        """Test dynamic pattern with numeric column value."""
        evaluator = RegexMatchEvaluator(patterns=r"Result:\s*{{number}}")
        result = evaluator(response="Result: 42", number=42)
        assert result[REGEX_MATCH] == 1.0

    def test_static_pattern_performance_optimization(self):
        """Test that static patterns are pre-compiled (no column refs)."""
        evaluator = RegexMatchEvaluator(patterns=r"(?i)ANSWER:\s*[A-D]")
    ...
    def test_dynamic_pattern_compilation_flag(self):
        ...
        evaluator = RegexMatchEvaluator(patterns=r"(?i)ANSWER:\s*{{correct}}")
