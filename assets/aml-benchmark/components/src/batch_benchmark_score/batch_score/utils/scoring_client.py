...
from .common.common import str2bool, get_base_url
from . import logging_utils as lu
from .logging_utils import get_events_client

import aiohttp, time, asyncio, json, uuid
...
class ScoringClient:
    ...
    async def score_until_completion(self, session: aiohttp.ClientSession, scoring_request: ScoringRequest, worker_id: str = "1") -> ScoringResult:
        cur_retries = 0
        base_wait = 1
        ...
        while scoring_result is None:
            try:
                scoring_result = await self.score_once(session=session, scoring_request=scoring_request, worker_id=worker_id)
    ...
    async def score_once(self, session: aiohttp.ClientSession, scoring_request: ScoringRequest, worker_id: str = "1") -> ScoringResult:
        if self.__routing_client and self.__quota_client:
            ...
            result = await self._score_once(session=session, scoring_request=scoring_request, worker_id=worker_id)
            return result

    async def _score_once(self, session: aiohttp.ClientSession, scoring_request: ScoringRequest, worker_id: str = "1") -> ScoringResult:
        response = None
        ...
        if self.__routing_client is not None:
            ...
            if len(scoring_request.request_history) > 0 and (scoring_request.request_history[-1][1] == 429 \
                                                             or scoring_request.request_history[-1][2] == "429" \
                                                             or (scoring_request.request_history[-1][1] == 404 and scoring_request.request_history[-1][3] == True)):
        ...
        lu.get_logger().debug("{}: Score request payload: <Redacted from application insights logging; see job logs directly. payload_correlation_id: {}>".format(worker_id, payload_correlation_id))
        print("{}: Score request payload [payload_correlation_id: {}]: {}".format(worker_id, payload_correlation_id, scoring_request.loggable_payload))
        ...
        try:
            ...
            async with session.post(url = target_endpoint_url, headers=headers, data=scoring_request.cleaned_payload, trace_request_ctx={"worker_id": worker_id}) as response:
                ...
                if response_status == 200:
                    ...
                    lu.get_logger().error("{}: Score failed -- internal_id: {} status: {} reason: {} -- response headers: {} | response payload: {}".format(
                        worker_id,
                        scoring_request.internal_id,
            ...
            lu.get_logger().error("{}: Score failed: asyncio.TimeoutError -- internal_id: {} x-ms-client-request-id: {}".format(
                worker_id, 
                scoring_request.internal_id,
                headers["x-ms-client-request-id"]))
            ...
            lu.get_logger().error("{}: Score failed: aiohttp.ServerConnectionError -- internal_id: {} x-ms-client-request-id: {}. Exception: {}".format(
                worker_id,
                scoring_request.internal_id,
                headers["x-ms-client-request-id"],
                e))
            ...
            lu.get_logger().error("{}: Score failed: aiohttp.ClientConnectorError -- x-ms-client-request-id: {}. Exception: {}".format(
                worker_id,
                headers["x-ms-client-request-id"],
                e))
            ...
            lu.get_logger().error("{}: Score failed: unhandled exception -- internal_id: {} x-ms-client-request-id: {}. Exception: {}".format(
                worker_id,
                scoring_request.internal_id,
                headers["x-ms-client-request-id"],
        ...
        scoring_request.request_history.append((endpoint_base_url, response_status, model_response_code, is_retriable))
        ...
        if response_status == 200:
            lu.get_logger().info("{}: Score succeeded at {} -- internal_id: {} x-ms-client-request-id: {} total_wait_time: {}".format(
                worker_id, 
