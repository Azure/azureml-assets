...
class DatasetPreprocessor(object):
    ...
    def validate(self) -> None:
        ...
        if self.input_dataset is None:
            mssg = "Path to jsonl file to load the dataset is not provided."
            raise BenchmarkValidationException._with_error(
                AzureMLError.create(BenchmarkValidationError, error_details=mssg)
            )
        if self.template is None and self.user_preprocessor is None:
            mssg = "Please provide the input to apply preprocessing logic either via template input or script_path."
            raise BenchmarkValidationException._with_error(
                AzureMLError.create(BenchmarkValidationError, error_details=mssg)
            )
        if self.user_preprocessor and not self.user_preprocessor.endswith(".py"):
            mssg = "Please provide python script containing your custom preprocessor logic."
            raise BenchmarkValidationException._with_error(
                AzureMLError.create(BenchmarkValidationError, error_details=mssg)
            )
        ...
        with open(self.output_dataset, mode="w", encoding="utf8") as f:
            for example in data:
                ...
                if self.encoder_config:
                    ...
                    out_dict[col_to_encode] = self.encoder_config.get(
                        str(out_dict.get(col_to_encode))
                    )
