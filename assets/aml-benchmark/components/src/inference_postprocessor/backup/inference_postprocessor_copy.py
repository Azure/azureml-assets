...
class InferencePostprocessor(object):
    ...
    def __init__(
        ...
        prediction_coumn_name: str = None,
        ...
        strip_prefixes: str = None,
        strip_suffixes: str = None,
        remove_prompt_prefix: bool = False,
        find_first: str = None,
        template: str = None,
        ...
        :param prediction_coumn_name: Name of prediction column/key.
        ...
        :param strip_prefixes: A list of characters separated by comma to remove from the beginning of the extracted answer.
        :param strip_suffixes: A list of characters separated by comma to remove from the end of the extracted answer.
        ...
        :param find_first: A list of strings to search for in the inference results. The first occurrence of each string \
            will be extracted. Provide a comma-separated list of strings. Example, for the following input: 
        ...
        self.prediction_coumn_name = prediction_coumn_name
    ...
    def validate(self) -> None:
        ...
        if self.prediction_coumn_name is None:
            mssg = (
    ...
    def read_ground_truth_dataset(self):
    ...
    def extract_using_template(self) -> None:
        ...
        pred_list = []
        if self.prediction_coumn_name in predicted_data[0].keys():
            key = self.prediction_coumn_name
        else:
            key = "0"
        ...
        if self.pred_probs_dataset:
            ...
            if self.label_map:
                ...
                probs_df = probs_df.rename(columns=self.label_map).add_prefix('probs_')
    ...
    def run(self) -> None:
        ...
        for row in predicted_data:
            ...
            if self.strip_prefixes:
                out_string = out_string.lstrip(self.strip_prefixes)
            if self.strip_suffixes:
                out_string = out_string.rstrip(self.strip_suffixes)
            pred_list.append(out_string)
        result_df[self.prediction_coumn_name] = pred_list
        if self.pred_probs_dataset:
            probs_df = pd.json_normalize(read_jsonl_files(resolve_io_path(self.pred_probs_dataset)))
...
def apply_regex_expr(self, text:str) -> str:
        ...
        if self.regex_expr:
            self.regex_expr = json.loads(json.dumps(self.regex_expr))
            # out_string = re.findall(self.regex_expr, text, flags=re.DOTALL)
            # if len(out_string)==0:
            #     return text
            # if isinstance(out_string[0], tuple):
            #     out_string = out_string[0]
