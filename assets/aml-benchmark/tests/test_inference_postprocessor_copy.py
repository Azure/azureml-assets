...
class TestInferencePostprocessorScript:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ('foo', ['abc_foo_def'], ['foo']), 
            ('foo', ['abcdef'], ['']), # no match
            ('foo', [''], ['']), # empty string has nothing
            ('foo,bar', ['bar_foo'], ['bar']), # returns earliest candidate found
            ('bar,foo', ['bar_foo'], ['bar']), # returns earliest candidate found
            ('foo,bar', ['abc_bar_foo', 'foo_bar'], ['bar', 'foo']), # returns earliest candidate found
            ('', ['foo'], ['foo']), # empty candidates don't trigger the find_first processor
            (None, ['foo'], ['foo']), # None candidates don't trigger the find_first processor
        ],
    )
    def test_apply_find_first(
        ...
        print(f"Input: {mock_completion_list}\nOutput: {output}\nExpected: {expected_completion_list}\n\n")
        assert output == expected_completion_list

    @pytest.mark.parametrize('separator, mock_completion_list, expected_completion_list', [
        ('|', ['foo|bar'], ['foo']),
        ('|', ['foo'], ['foo']),
        ('|', ['|foo'], ['']),
        ('|', ['|'], ['']),
        ...
        ('|||', ['foo|||', 'bar'], ['foo', 'bar']),
        (' ', ['foo bar'], ['foo']),
        (None, ['foo'], ['foo']),  # test if separator is None
        (None, ['foo|bar'], ['foo|bar']), # test if separator is None
        ('', ['foo'], ['foo']), # test if separator is empty string
        ('', ['foo|bar'], ['foo|bar']),  # test if separator is empty string
    ])
    ...
    @pytest.mark.parametrize('regex, mock_completion_list, expected_completion_list', [
        ('foo', ['foo'], ['foo']),
        (' (.*)', ['foo'], ['foo']),
        (' ([-+]?\\d+)[.]{0,1}$', ['abc 123'], ['123']),
        (" ([-+]?\d+)[.]{0,1}$", ['abc 123'], ['123']),
        ...
        ('answer is (Yes|No)', ['answer is Maybe'], ['answer is Maybe']), # NOTE: this is not intiuitive!
        ('"Answer: ([^\\n]+?)(?:\\.|\\n|$)"', ['"Answer: ] ) )."'], ['] ) )']),
        ('\d+', ['foo123'], ['123']),
    ])
    ...
    @pytest.mark.parametrize('mock_completion_list, expected_completion_list', [
        (['foo123'], ['123']),  # regex = '\d+' or '-?[0-9|\d]+\.?[0-9|\d]*'
        (['foo123', '456foo123'], ['123', '456']), # regex = '\d+' or '-?[0-9|\d]+\.?[0-9|\d]*'
        (['bar', 'foo456', '', 'foo'], ['0', '456', '0', '0']), # regex = '\d+', # 0 is the default value, this is what extract_regex in Babel's code does not apply
        (['4foo3'], ['4']),  # regex = '-?[0-9|\d]+\.?[0-9|\d]*'
        (['-4foo3'], ['-4']), # regex = '-?[0-9|\d]+\.?[0-9|\d]*'
        (['foo-123'], ['-123']), # regex = '-?[0-9|\d]+\.?[0-9|\d]*'
        (['foo -123'], ['-123']), # regex = '-?[0-9|\d]+\.?[0-9|\d]*'
        (['42'], ['42']),
        (['42foo'], ['42']),
        ...
        (['-.456'], ['-.456']), # regex = '-*[0-9|\d]*\.?[0-9|\d]*'
        (['--.456'], ['-.456']),  # regex = '(-*[0-9|\d]*\.?[0-9|\d]*)' worked but expected output wants the removal of noise i.e. -- replace with -
        (['+56'], ['56']),
        ([',56'], ['56']),
        (['56,'], ['56']),
        (['5,6'], ['5']),
    ...
    @pytest.mark.parametrize('mock_completion_list, expected_completion_list', [
        ...
        (['bar', '123foo456', '', 'foo'], ['0', '456', '0', '0']), # 0 is the default value
        (['4foo3'], ['3']),
        (['42'], ['42']),
        (['42foo'], ['42']),
        (['123.456foo7.8'], ['7.8']),
    ...
    def test_apply_extract_number_last(
        ...
        output = []
        for input in mock_completion_list:
            output.append(obj.apply_extract_number(text=input, strategy='last'))
        assert output == expected_completion_list
    ...
    @pytest.mark.parametrize('prefixes, mock_completion_list, expected_completion_list', [
        ('<prefix>', ['<prefix>foo'], ['foo']),
        ('<wrong_prefix>', ['<prefix>foo'], ['<prefix>foo']),
        ('<prefix>', ['<prefix>foo', '<prefix>bar'], ['foo', 'bar']),
        ('', ['foo'], ['foo']),
        ('<prefix>', [' <prefix>foo'], [' <prefix>foo']), # sensitive to whitespace
    ...
    @pytest.mark.parametrize('prompt_prefix, mock_completion_list, expected_completion_list', [
        ('<prompt_prefix>', ['<prompt_prefix>foo'], ['foo']),
        ('<prompt_prefix, has commas>', ['<prompt_prefix, has commas>foo'], ['foo']),
        ('<prompt_prefix>', ['<other_prefix>foo'], ['<other_prefix>foo']),
        ('', ['foo'], ['foo']),
    ])
    def test_remove_prompt_prefix(
        self, prompt_prefix: str, mock_completion_list: List, expected_completion_list: List
    ) -> None:
