...
YAML_SINGLE_ENDPOINT_VESTA_CHAT_COMPLETION = {"jobs": { JOB_NAME: {
    "inputs": {
        # (12/22/2023)
        # This e2e test sends only three requests, so it needs very little quota.
        # The gptv-eval-global pool is backed by a single MIR endpoint and has a quota of 300 concurrent requests.
        # This is much lower than the other pools.
        # For example, the dv3-global pool supports 68 million concurrent requests.
        # The quota service is implemented as a distributed rate limiter.
        # By design, distributed rate limiters are not well suited to accurately enforce small quotas.
        # So for the gptv-eval-global pool, we target the endpoint directly instead of going through the quota service.
        "online_endpoint_url": "https://prom-vesta-neweval.centralus.inference.ml.azure.com/v1/rainbow/chat/completions"
    }
}}}
...
@pytest.mark.e2e
@pytest.mark.timeout(15 * 60)
def test_gated_vesta_chat_completion_batch_score_single_endpoint(batch_score_vesta_chat_completion_yml_component):
    set_component(*batch_score_vesta_chat_completion_yml_component, component_config=YAML_COMPONENT, job_name=JOB_NAME)
    display_name = {"display_name": f"{RUN_NAME}_smoke"}
    yaml_update = deep_update(YAML_COMPONENT,
                              YAML_SMOKE_VESTA_CHAT_COMPLETION_BASE64_IMAGE_TEST_DATA_ASSET,
                              YAML_APPLICATION_INSIGHTS,
                              YAML_SINGLE_ENDPOINT_VESTA_CHAT_COMPLETION,
                              YAML_DISALLOW_FAILED_REQUESTS,
                              display_name)
    _submit_job_and_monitor_till_completion(
        ml_client=pytest.ml_client,
...
@pytest.mark.e2e
@pytest.mark.timeout(15 * 60)
def test_gated_vesta_chat_completion_with_image_urls_batch_score_single_endpoint(batch_score_vesta_chat_completion_yml_component):
    set_component(*batch_score_vesta_chat_completion_yml_component, component_config=YAML_COMPONENT, job_name=JOB_NAME)
    display_name = {"display_name": f"{RUN_NAME}_smoke"}
    yaml_update = deep_update(YAML_COMPONENT,
                              YAML_SMOKE_VESTA_CHAT_COMPLETION_IMAGE_URL_TEST_DATA_ASSET,
