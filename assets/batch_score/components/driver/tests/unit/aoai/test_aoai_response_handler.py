...
def test_handle_response_returns_success_result(mock_run_context):
    ...
    http_response = HttpScoringResponse(
        status=200,
        payload={"usage": {"prompt_tokens": test_prompt_tokens, "completion_tokens": test_completion_tokens, "total_tokens": test_prompt_tokens + test_completion_tokens}})
    scoring_request = _get_test_scoring_request()   
    expected_request_completed_event = _get_expected_request_completed_event(response_handler, scoring_request, http_response)
...
def test_handle_response_non_retriable_status_code_returns_failure(status_code, tally_handler_enable, mock_run_context):
    # Arrange
    http_response = HttpScoringResponse(status=status_code, payload=['hello'])
    scoring_request = _get_test_scoring_request()
...
def _get_expected_request_completed_event(
    ...
    return BatchScoreRequestCompletedEvent(
        ...
        model_response_code = None if not http_response.headers else http_response.headers.get("ms-azureml-model-error-statuscode", None),
        prompt_tokens = test_prompt_tokens if (http_response.payload is not None and "usage" in http_response.payload) else None,
        completion_tokens = test_completion_tokens if (http_response.payload is not None and "usage" in http_response.payload) else None,
