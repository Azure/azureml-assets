...
from src.batch_score.root.common.parallel.request_metrics import RequestMetrics
from src.batch_score.root.common.parallel.worker import QueueItem, Worker
...
async def test_successful_scoring_appends_result_no_segmentation(
    ...
    monkeypatch.setattr(
        "src.batch_score.root.common.scoring.generic_scoring_client.GenericScoringClient.score",
        mock_score)
    with patch("src.batch_score.root.common.telemetry.events.event_utils.emit_event") as mock_emit_event:
        with patch("src.batch_score.root.common.scoring.segmented_score_context.SegmentedScoreContext"
...
async def test_successful_scoring_appends_result_with_segmentation(
    ...
    monkeypatch.setattr("src.batch_score.root.common.scoring.segmented_score_context"
                        ".SegmentedScoreContext.score_next_segment", mock_score)
    with patch("src.batch_score.root.common.telemetry.events.event_utils.emit_event") as mock_emit_event:
        with patch("src.batch_score.root.common.scoring.segmented_score_context"
                   ".SegmentedScoreContext.processed_segments_count",
                   new_callable=PropertyMock) as mock_processed_segments_count:
            with patch('src.batch_score.root.common.scoring.segmented_score_context.SegmentedScoreContext.has_more',
