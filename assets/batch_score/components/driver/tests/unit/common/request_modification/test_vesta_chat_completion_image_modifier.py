...
@pytest.mark.parametrize("request_payload, expected_result", [
    ({"messages": [{"role": "user", "content": [{"image": "base64encoded"}, "Transcribe this image please"]}]}, True),
    ({"messages": [{"role": "user", "content": [{"image": "base64encoded"}, "Transcribe this image please"]}, {"hello": "world"}]}, False),
    ({"messages": [{"hello": "world"}]}, False),
    ({"invalid": [{"role": "user", "content": [{"image": "base64encoded"}, "Transcribe this image please"]}]}, False)
])
def test_is_vesta_chat_completion_payload(request_payload, expected_result):
    assert VestaChatCompletionImageModifier.is_vesta_chat_completion_payload(request_obj=request_payload) == expected_result
...
def test_modify(mock_get_logger, make_vesta_chat_completion_image_modifier, mock__b64_from_url):
    ...
    assert modified_request_obj["messages"][2]["content"][0]["image_url"]["url"] == "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAACAAAAAgACAIAAA…kSuQmCC"
