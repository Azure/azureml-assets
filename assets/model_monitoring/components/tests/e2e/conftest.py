...
@pytest.fixture(scope="session", autouse=True)
def publish_generation_safety_quality_model_monitor_component(
    main_worker_lock,
    publish_command_components,
    model_monitoring_components,
    components_directory,
    ...
    """Publish the GSQ model monitor pipeline component to the test workspace."""
    if not _is_main_worker(main_worker_lock):
        return

    print_header("Publishing GSQ Model Monitor")
    out_directory = os.path.join(root_temporary_directory, "command_components")
    os.makedirs(out_directory, exist_ok=True)
    ...
    for component in model_monitoring_components:
        ...
        component["jobs"]["compute_histogram"][
            "component"
        ] = f"azureml:gsq_annotation_compute_histogram:{asset_version}"
        component["jobs"]["compute_metrics"][
            "component"
        ] = f"azureml:gsq_annotation_compute_metrics:{asset_version}"
        component["jobs"]["output_signal_metrics"][
            "component"
...
@pytest.fixture(scope="session", autouse=True)
def publish_gsq_model_monitor_component(
    main_worker_lock,
    publish_command_components,
    model_monitoring_components,
    components_directory,
...
def release_lock(
    ...
    publish_generation_safety_quality_model_monitor_component,
    publish_gsq_model_monitor_component,
    publish_command_components,
    main_worker_lock,
):
