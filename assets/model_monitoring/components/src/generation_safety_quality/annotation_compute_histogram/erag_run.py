...
def write_signal_output(signal_output_path):
    des_path = f"{signal_output_path.rstrip()}/signals"
    fs = AzureMachineLearningFileSystem(des_path)
    print("des path:", des_path)
    gsq_signal_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), "gsq_signal.json")
    fs.upload(
        lpath=gsq_signal_path,
        rpath="",
        recursive=True,
    )
...
def run():
    ...
    parser.add_argument("--signal_output", type=str, required=True)
    ...
    #     '--signal_output', f'azureml://subscriptions/79a1ba0c-35bb-436b-bff2-3074d5ff1f89/resourcegroups/azureml-rag-ci/workspaces/azureml-rag-westus2/datastores/workspaceblobstore/paths/azureml/{Run.get_context().id}/signal_output/',
    ...
    # write mock signal output to pass the POC pipeline
    write_signal_output(args.signal_output)
