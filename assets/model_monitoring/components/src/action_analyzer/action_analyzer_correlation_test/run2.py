...
def perform_correlation_test(good_group_df,
    ...
    print("good queries: ")
    good_group_df = good_group_df.withColumn("Debugging", get_debugging_column(col(ACTION_METRICS_COLUMN), col(PROPERTIES_COLUMN)))
    print("\n".join(good_group_df.select("Debugging").rdd.flatMap(lambda x: x).collect()))
    print("bad queries: ")
    bad_group_df = bad_group_df.withColumn("Debugging", get_debugging_column(col(ACTION_METRICS_COLUMN), col(PROPERTIES_COLUMN)))
