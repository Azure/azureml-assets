...
class Component:
    ...
    def increment_component(self):
        ...
        text_data = text_data.replace(
            f"version: {old_version}", f"version: {self.version}"
        )
...
class Pipeline(Component):
    ...
    def increment_component(
        self, comp_assets_all: Dict[str, Component], update_all: bool
    ):
        ...
        for s_c in yaml_data["jobs"]:
            ...
            try:
                ...
                print(
                    f"[{self.name}] Cannot find sub-component {e}, assuming that it comes from a skipped directory."
                )
                continue

            if sub_comp.version != new_version:
                needs_update = True
                new_version_str = sub_comp_str.replace(sub_comp.version, new_version)

                text_data = text_data.replace(sub_comp_str, new_version_str)
...
def main_wrapper(folder_path: str, update_all: bool) -> None:
    ...
    print(
        f"Updating Pipeline and linked Component versions (Count:{len(pipe_assets_all)})...."
    )
...
def validate_environment(compt_name: str, env_value: str) -> None:
    ...
    if len(match) <= 0:
        match = re.findall(
            r"([^:]+):([^\:\@]+)[\:\@]([^\:\@]+)", env_value, flags=re.IGNORECASE
        )
...
def parse_component(comp_ref: str) -> Component:
    ...
    if len(match) <= 0:
        match = re.findall(r"([^:]+):([^:]+):([\.\d]+)", comp_ref, flags=re.IGNORECASE)
...
target_fields = {"version": lambda x: str(x), "name": lambda x: x, "jobs": parse_jobs}

validate_fields = {"environment": validate_environment}
...
) -> Tuple[Dict[str, Component], Dict[str, Pipeline]]:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--update_all", action=BooleanOptionalAction, required=False, default=False
    )
