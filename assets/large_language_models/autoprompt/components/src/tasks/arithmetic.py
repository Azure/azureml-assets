...
logger = get_logger("arithmetic")


class ArithmeticQA(Task):
    """Arithmetic QA class."""

    def __init__(self, meta_prompts=[]):
        ...
        self.prompt_constructor = ConstructPrompt(TaskTypes.arithmetic).construct_prompt
        self.best_prompt = None
        self.phase_2_prompt = "\nTherefore, the answer (arabic numerals) is"

    def find_best_prompt(self, X, y, valid_x, valid_y, text_keys, **kwargs):
        """Find Best Prompt."""
        with track_activity(logger, Activities.ARITHMETIC, custom_dimensions=CUSTOM_DIMENSIONS) as arithmetic_activity:
            ...
            with track_activity(logger, Activities.EVALUATE_PROMPTS,
                ...
                for base_prompt in self.meta_prompts:
                    ...
                    try:
                        y_pred_p1 = get_predictions(zs_texts, **kwargs)
                    ...
                    try:
                        metrics = compute_metrics(task_type="qa", y_test=y.tolist(), y_pred=y_pred)
                    except Exception as e:
                        make_and_log_exception(
            ...
            try:
                metrics = compute_metrics(task_type="qa", y_test=valid_y.tolist(), y_pred=valid_preds)
            except Exception as e:
                make_and_log_exception(
                    error_cls=ComputeMetricsInternalError,
                    exception_cls=ComputeMetricsException,
                    logger=logger,
                    inner_exception=e,
                    activity_logger=arithmetic_activity,
                    message_params={"error": repr(e)}
                )
