...
Image: `public/azureml/curated/ai-ml-automl-dnn-text-gpu-ptca:51`

Base image: `mcr.microsoft.com/azureml/openmpi5.0-ubuntu24.04:20260707.v1` (resolved from `{{latest-image-tag}}`). The MCR base image had no downloadable SBOM referrer, so it was mirrored and VCM generated `base-sbom.json` and `base-vulnerabilities.json` from that mirror.

Scan artifacts for `vulnscan1779267129n2.azurecr.io/public/azureml/curated/ai-ml-automl-dnn-text-gpu-ptca:test-fix` are retained locally as `sbom.json` and `vulnerabilities.json`; base image artifacts are retained as `base-sbom.json` and `base-vulnerabilities.json`.
...
| 5015223 / GHSA-wf93-45jw-7689 | `pip` | Base-image Python prefix at `/opt/conda/envs/ptca` | No; this Dockerfile upgrades independent Python prefixes | `context\Dockerfile` | `pip>=26.1.2` |
...
| 6035952 / USN-8512-1 | `gzip` | Base-image OS package | No; base SBOM has `1.12-1ubuntu3.1`, and this Dockerfile upgrades it | `context\Dockerfile` | `1.12-1ubuntu3.2` |
