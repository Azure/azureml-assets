...
from local_constants import TelemetryConstants
...
class AppInsightsPIIStrippingFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        record.message = record.msg = '\n'.join([
            'Type: {}'.format(properties.get('error_type', local_constants.ExceptionTypes.Unclassified)),
...
def _get_error_details(
    ...
    error_code = local_constants.ExceptionTypes.Unclassified
    error_type = local_constants.ExceptionTypes.Unclassified
    ...
    if isinstance(exception, (AzureMLException)):
        try:
            ...
            error = serialized_ex.get(
                "error", {
                    "code": local_constants.ExceptionTypes.Unclassified,
                    "inner_error": {}, "target": default_target
                }
            )

            # This would be the complete hierarchy of the error
            error_code = str(error.get("inner_error", local_constants.ExceptionTypes.Unclassified))
