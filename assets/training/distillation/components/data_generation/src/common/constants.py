...
Guidelines:
- The first summary should be long (4-5 sentences, ~80 words) yet highly non-specific, containing little information \
beyond the entities marked as missing. Use overly verbose language and fillers (e.g., "this article discusses") to \
reach ~80 words.
...
Answer only in JSON. The JSON should be a list (length 4) of dictionaries whose keys are "Missing_Entities" and \
"Denser_Summary". Ensure the JSON starts with a square bracket [, ends with a square bracket ], and each dictionary \
within the array is separated by a comma. The JSON should be syntactically correct and properly formatted. For \
example:
[
