## Pipeline RFT Trainer

### Name

pipeline_rl_finetune
...
Pipeline for Multi-Strategy Reinforcement Learning Training of Large Language Models. Supports GRPO (Group Relative Policy Optimization) and REINFORCE++ algorithms with distributed training capabilities.
...
| instance_type_finetune          | Instance type to be used for finetune component in case of serverless compute, eg. standard_nc24rs_v3. The parameter compute_finetune must be set to 'serverless' for instance_type to be used  | string  | Standard_ND96isr_H100_v5 | True     | NA   |
| shm_size_finetune               | Shared memory size to be used for finetune component. It is useful while using Nebula (via DeepSpeed) which uses shared memory to save model and optimizer states                              | string  | 5g                      | True     | NA   |
...
| Name                        | Description                                                                                                                                                               | Type    | Default | Optional | Enum                                    |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- | ------- | -------- | --------------------------------------- |
| algorithm_adv_estimator     | Advantage estimator algorithm. Valid values: grpo (Group Relative Policy Optimization), reinforce_plus_plus (REINFORCE++ with global advantage normalization)           | string  | grpo    | True     | ['grpo', 'reinforce_plus_plus']        |
