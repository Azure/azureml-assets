...
def get_parser():
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="The token to access private models from Hugging Face. You can create an access token by following the instructions [here](https://huggingface.co/docs/hub/security-tokens). The token must have at least read access to the model. This parameter is optional if you are using public models.",
    )
    ...
    if args.huggingface_id is not None:
        ...
        if validate_huggingface_id(args.model_name):
            ...
            if args.pytorch_model_path is None and args.mlflow_model_path is None:
                ...
                token_args = {"token": args.huggingface_token.strip()} if args.huggingface_token else {}
                logger.info(f"Pulling model {args.model_name} from Hugging Face to {model_dir} using token_args {token_args}")
                # download config, tokenizer, and model into model_dir
                # Download config, tokenizer, and model from Hugging Face
                config = AutoConfig.from_pretrained(args.model_name, **token_args)
                tokenizer = AutoTokenizer.from_pretrained(args.model_name, **token_args)
                model = AutoModelForCausalLM.from_pretrained(args.model_name, config=config, **token_args)
