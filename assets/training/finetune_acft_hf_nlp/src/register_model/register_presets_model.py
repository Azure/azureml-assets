...
def get_dataset_relative_path(dataset):
    ...
    source = [(store['datastoreName'], store['path']) for store in step_arguments['datastores']]
...
def create_v1_dataset(run_id, ws):
    ...
    finetuned_model_dataset = Dataset.File.from_files(path=[(datastore, rel_path)], validate=False)
    finetuned_model_dataset = finetuned_model_dataset.register(
        workspace=ws,
        name="register_model_dataset_" + run_id
    )
        ...
        try:
            ...
            if resp.status_code == 409:
                ...
                if i == retry_count:
                    ...
                    logger.error('Attempting to retry again with autoVersion set to True.')
...
def register_model(workspace, model_name, model_version, dataset_id, run_id, is_v2, tags=None, properties=None, description=None, model_format="CUSTOM"):
    ...
    if tags:
        try:
            ...
            raise ValueError('Error with JSON serialization for tags, '
    ...
    if properties:
        try:
            ...
            raise ValueError('Error with JSON serialization for properties, '
                            'be sure they are properly formatted.')
    ...
    resp = submit_rest_request(get_requests_session().post, request_url, request_body,
                                request_params, request_headers, use_auto_version=use_auto_version)
...
def registermodel_entrypoint(
    ...
    engine_controller_manifest_path = "azureml/{}/output_model/manifest.enginecontroller.json".format(run_id)
    ...
    resp_json = register_model(workspace=ws,
