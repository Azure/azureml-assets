...
def set_flash_attention(args: Namespace):
    ...
    if (
        ...
        setattr(args, "apply_flash_attention", False)
        setattr(args, "flash_attention_version", -1)
    if args.precision == 32 and args.finetune_config.get("load_model_kwargs", {}).get("use_flash_attention_2", False) is True:        
        ...
        raise ACFTValidationException._with_error(
                    AzureMLError.create(
                        ACFTUserError,
                        pii_safe_message=(
                            f"Flash Attention is not supported with 32-bit precision."
                        )
                    )
                )
