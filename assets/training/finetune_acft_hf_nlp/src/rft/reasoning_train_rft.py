...
║                                                                               ║
║   ┌────────┐         ┌──────────┐       ┌───────────┐       ┌────────────┐    ║
║   │  DATA  │─────▶  │ SAMPLER   │─────▶│ GRADER    │─────▶│  TRAINER   │    ║
║   │ Input  │         │ (Rollout)│       │(Algorithm)│       │ (GRPO/etc) │    ║
║   └────────┘         └──────────┘       └───────────┘       └────────────┘    ║
║       │                │                  │                  │                ║
║       │                │                  │                  │                ║
║   Prompts        Generates N         Computes          Updates Model          ║
║   from JSONL     Responses per       Rewards &         via Policy             ║
║   dataset        Prompt using        Advantages        Gradient               ║
║                  LLM + Sampling                                               ║
║                                                                               ║
║   Key Controls:                                                               ║
║   • TRAINER  → epochs, checkpoints, validation                                ║
║   • SAMPLER  → temperature, top_k/top_p, sampling count                       ║
║   • GRADER   → reward functions, KL penalties, advantage estimation           ║
║                                                                               ║
╚═══════════════════════════════════════════════════════════════════════════════╝
"""
import argparse
...
def parse_args():
    ...
    # ║  ┌─────────────────────────────────────────────────────────────────────┐  ║
    # ║  │  The SAMPLER (Rollout) generates responses from the model:          │  ║
    # ║  │                                                                     │  ║
    # ║  │   Input Prompt                                                      │  ║
    # ║  │        │                                                            │  ║
    # ║  │        ▼                                                            │  ║
    # ║  │   ┌────────────────────┐  rollout_temperature → controls randomness │  ║
    # ║  │   │  vLLM Engine       │  top_k/top_p → sampling strategies         │  ║
    # ║  │   │  ┌──────────────┐  │  rollout_n → number of samples per prompt  │  ║
    # ║  │   │  │ 🎲 Sample    │  │  do_sample → enable/disable sampling       │  ║
    # ║  │   │  │ Generation   │  │                                             │  ║
    # ║  │   │  └──────────────┘  │                                             │  ║
    # ║  │   └────────────────────┘                                            │  ║
    # ║  │        │                                                            │  ║
    # ║  │        ▼                                                            │  ║
    # ║  │   Generated Responses → [Response₁, Response₂, ..., ResponseN]      │  ║
    # ║  └─────────────────────────────────────────────────────────────────────┘  ║
    # ╚═══════════════════════════════════════════════════════════════════════════╝
    #
    # ═══════════════════════════════════════════════════════════════════════════
    #                    SAMPLER DETAILED FUNCTIONALITY
    # ═══════════════════════════════════════════════════════════════════════════
    #
    # The SAMPLER (also called "Rollout") is responsible for generating diverse
    # responses from the language model. It's a critical component that explores
    # the solution space and provides candidate outputs for the grader to evaluate.
    ...
    parser.add_argument("--rollout_load_format", type=str, default="dummy", help="Model weight loader")
    ...
    parser.add_argument("--ref_fsdp_forward_prefetch", type=bool,
                        default=False, help="Ref FSDP1 forward prefetch")
    ...
    parser.add_argument("--data_video_key", type=str, default="videos",
                        help="Video field in multimodal dataset")
    ...
    # Note: trainer_critic_warmup is hardcoded to 0 (no critic warmup for GRPO)
    parser.add_argument("--trainer_logger", type=str, default='["console"]', help="Logger")
    parser.add_argument("--trainer_log_val_generations", type=int, default=0,
                        help="Number of validation generations to log")
    parser.add_argument("--trainer_project_name", type=str, default="verl_grpo_example_geo3k", help="Project name")
    parser.add_argument("--trainer_experiment_name", type=str, default="qwen2_5_vl_7b_function_rm",
    ...
    parser.add_argument("--trainer_save_contents", type=str, default='["hf_model"]', help="Save Contents")
    ...
    parser.add_argument("--trainer_resume_mode", type=str, default="auto",
                        help="Resume mode: auto, disable, or resume_path")
...
def setup_ray_cluster():
    ...
    os.environ["RAY_ENABLE_STATS"] = "0"
    os.environ["RAY_DISABLE_PRINT_STATS"] = "1"
    os.environ["RAY_SCHEDULER_EVENTS"] = "0"
    os.environ["RAY_ENABLE_RESOURCE_SUMMARY"] = "0"
    os.environ["RAY_DEDUP_LOGS"] = "1"  # Enable log deduplication
    os.environ["RAY_LOG_DEDUP_PERIOD_S"] = "3600"  # Deduplicate for 1 hour
    os.environ["RAY_monitor_autoscaler_log_level"] = "ERROR"  # Suppress monitor logs

    # Additional aggressive suppression
    os.environ["RAY_DISABLE_DOCKER_CPU_WARNING"] = "1"
    os.environ["RAY_DISABLE_MEMORY_MONITOR"] = "1"
    os.environ["AUTOSCALER_UPDATE_INTERVAL_S"] = "999999"  # Effectively disable autoscaler updates

    # Suppress autoscaler at the Python logging level BEFORE Ray starts
    import logging
    logging.getLogger("ray.autoscaler").setLevel(logging.CRITICAL)
    logging.getLogger("ray._private.monitor").setLevel(logging.CRITICAL)
    logging.getLogger("ray._private.resource_spec").setLevel(logging.CRITICAL)
...
def main():
    ...
    try:
        ...
        import os
        ray_logging.getLogger("ray").setLevel(ray_logging.WARNING)
        ray_logging.getLogger("ray.serve").setLevel(ray_logging.WARNING)
        ray_logging.getLogger("ray.rllib").setLevel(ray_logging.WARNING)
        ray_logging.getLogger("ray.tune").setLevel(ray_logging.WARNING)
        ray_logging.getLogger("ray.data").setLevel(ray_logging.WARNING)
        ray_logging.getLogger("ray.workflow").setLevel(ray_logging.WARNING)
        ray_logging.getLogger("ray.autoscaler").setLevel(ray_logging.CRITICAL)
        ray_logging.getLogger("ray._private.resource_spec").setLevel(ray_logging.CRITICAL)
        ray_logging.getLogger("ray._private.usage").setLevel(ray_logging.CRITICAL)
        ray_logging.getLogger("ray._private.monitor").setLevel(ray_logging.CRITICAL)
        ray_logging.getLogger("ray.autoscaler._private.monitor_manager").setLevel(ray_logging.CRITICAL)

        # Disable propagation to prevent logs from bubbling up
        ray_logging.getLogger("ray.autoscaler").propagate = False
        ray_logging.getLogger("ray._private.monitor").propagate = False
        ...
        # Log all parsed arguments for debugging and transparency
        ...
        if args.pypi_packages_override is not None and \
            ...
            parts = [part.strip() for part in override_str.split(',') if part.strip()]
            logger.info(f"Split into {len(parts)} part(s) to process")

            for idx, part in enumerate(parts, 1):
                logger.info(f"Processing part {idx}/{len(parts)}: {part}")

                # Check for environment variables at the beginning (only if part contains pip/pip3)
                env_vars = {}
                part_words = part.split()
                pip_cmd_start_idx = 0

                # Only look for env vars if this looks like a pip command
                if any(word in ["pip", "pip3"] for word in part_words):
                    for i, word in enumerate(part_words):
                        if '=' in word and not word.startswith('-') and word not in ["pip", "pip3"]:
                            # Check if this word comes before pip/pip3
                            remaining_words = part_words[i:]
                            if any(w in ["pip", "pip3"] for w in remaining_words):
                                ...
                                break
                        elif word in ["pip", "pip3"]:
                            # Found the pip command, stop looking for env vars
                            break
                ...
                if part.startswith(("pip ", "pip3 ")):
                    ...
                    if cmd_parts[0] in ["pip", "pip3"]:
                        cmd_parts = cmd_parts[1:]  # Remove pip/pip3

                    # Check if this is install or uninstall command
        ...
        cmd = [
            ...
            "data.return_raw_chat=True",  # Required for agent-based rollout to access raw_prompt
        ...
        # Add FSDP mixed precision config hardcoded to bf16
        cmd.append("+actor_rollout_ref.actor.fsdp_config.mixed_precision.param_dtype=bf16")
        cmd.append("+actor_rollout_ref.actor.fsdp_config.mixed_precision.reduce_dtype=bf16")
        cmd.append("+actor_rollout_ref.actor.fsdp_config.mixed_precision.buffer_dtype=bf16")
        ...
        cmd.extend([
            f"trainer.default_local_dir={quote_if_needed(args.intermediate_folder)}",
            f"trainer.project_name={args.trainer_project_name}",
        ])
        ...
        # Create a custom filter to suppress Ray verbose stats output
        ...
        class RayLogFilter:
            ...
            def __init__(self, stream):
                ...
                self.suppress_patterns = [
                    ...
                    "Recent failures:",
                    "The autoscaler took",
                    "to complete the update iteration",
                    "to fetch the list of non-terminated nodes",
                    "NCCL INFO",
                    "(dashboard)",
                    "(dashboard_agent)",
                    "LNXSYBUS",
                ...
                self.in_resource_block = False
                self.skip_next_lines = 0

            def write(self, text):
                # Skip if we're in the middle of skipping lines
                if self.skip_next_lines > 0:
                    self.skip_next_lines -= 1
                    return

                # Check if this is a resource summary header
                if "Resources" in text and "-------" in text:
                    self.in_resource_block = True
                    self.skip_next_lines = 20  # Skip next 20 lines of resource details (increased)
                    return

                # Check if this is the autoscaler status header
                if "Autoscaler status:" in text or "======== Autoscaler" in text:
                    self.in_resource_block = True
                    self.skip_next_lines = 25  # Skip entire autoscaler block
                    return
                ...
                # Reset resource block flag if we see a non-resource line
                if self.in_resource_block and not any(p in text for p in ["Total", "Node", "Memory", "CPU", "GPU"]):
                    self.in_resource_block = False

                if not self.in_resource_block:
                    self.stream.write(text)

            def flush(self):
                self.stream.flush()

        # Apply RayLogFilter to capture and filter output in real-time
        # This allows streaming logs while suppressing verbose Ray stats and NCCL INFO
        stdout_filter = RayLogFilter(sys.stdout)

        # Run subprocess with Popen to stream output through the filter
        ...
        # Stream output line by line through the filter
        try:
            for line in process.stdout:
                stdout_filter.write(line)
                stdout_filter.flush()
        except Exception as e:
            logger.error(f"Error while streaming output: {e}")
        ...
        # Copy the latest checkpoint to model_output
        logger.info("Copying latest checkpoint to model_output...")
        try:
            # Read latest_checkpointed_iteration.txt to get the checkpoint iteration number
            latest_iteration_file = os.path.join(args.intermediate_folder, CHECKPOINT_FILE_NAME)
            ...
            logger.info(f"Latest checkpointed iteration: {iteration_number}")

            # Construct the source checkpoint folder path
            checkpoint_folder = f"global_step_{iteration_number}"
            source_path = os.path.join(args.intermediate_folder, checkpoint_folder)
            ...
            # Copy all contents from checkpoint folder to model_output
            for item in os.listdir(source_path):
                src_item = os.path.join(source_path, item)
                dst_item = os.path.join(args.output_dir, item)
            ...
            logger.info(f"Successfully copied checkpoint contents to {args.output_dir}")
        except ValueError as e:
            _log_user_error(f"ValueError occurred while running RFT training script.{e}")
        except KeyError as e:
            _log_user_error(f"KeyError occurred while running RFT training script. {e}")
        except Exception as e:
