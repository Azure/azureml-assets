...
# Initialize logger
logger = get_logger_app("azureml.acft.contrib.hf.scripts.src.draft_model.train_draft_model")

# Other constants
COMPONENT_NAME = "ACFT-Draft-Model-Training"
        ...
        if self.rank == 0:
            ...
            try:
                ...
                if self.azureml_run and "OfflineRun" in self.azureml_run.id:
                    ...
                    logger.info("AzureML logging initialized for draft-model training")
                    ...
                    if self.log_metrics_at_root:
                        ...
                        if parent_run:
                            try:
                                ...
                                logger.warning(f"Warning: Failed to log to parent run: {e}")
...
def find_model_config_path(root_path):
    ...
    config_path = os.path.join(root_path, CONFIG_JSON)
    if os.path.isfile(config_path):
        print_on_rank0(f"Found {CONFIG_JSON} at: {root_path}")
        return root_path

    # Search in subdirectories
    if os.path.isdir(root_path):
        for dirpath, dirnames, filenames in os.walk(root_path):
            if CONFIG_JSON in filenames:
                print_on_rank0(f"Found {CONFIG_JSON} in subdirectory: {dirpath}")
                return dirpath

    print_on_rank0(f"Warning: {CONFIG_JSON} not found in {root_path} or its subdirectories")
    return root_path
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Path to a checkpoint directory to resume training from.\
                Used when resume is true and no checkpoints exist in output folder,\
                or when resume is false to initialize from a pretrained draft model checkpoint.",
    )
    ...
    parser.add_argument("--verbose", type=str_to_bool, default=False)
    parser.add_argument("--profile", type=str_to_bool, default=False)
    parser.add_argument("--profile_start_step", type=int, default=30)
    parser.add_argument("--profile_num_steps", type=int, default=4)
    parser.add_argument("--profile_record_shapes", type=str_to_bool, default=False)
    ...
    # Validate chat_template against TEMPLATE_REGISTRY
    from specforge.data.template import TEMPLATE_REGISTRY
    valid_templates = TEMPLATE_REGISTRY.get_all_template_names()
    if args.chat_template not in valid_templates:
        error_msg = (
            f"ERROR: Invalid chat_template '{args.chat_template}'. "
            f"Supported templates: {valid_templates}"
        )
...
def main():
    ...
    set_logging_parameters(
        task_type="draft-model-training",
    ...
    logger.info("Starting Draft Model training.")
    logger.info(f"Component version: {VERSION}")

    # Validate chat_template
    from specforge.data.template import TEMPLATE_REGISTRY
    valid_templates = TEMPLATE_REGISTRY.get_all_template_names()
    if args.chat_template not in valid_templates:
        error_msg = (
        ...
        sys.exit(1)
    logger.info(f"Using chat_template: {args.chat_template}")
    ...
    try:
        ...
        logger.info(f"Distributed setup: LOCAL_RANK={local_rank}, WORLD_SIZE={world_size}, Available GPUs={num_gpus}")
        ...
        _log_system_error(f"ERROR: Failed to initialize distributed environment: {e}")
        sys.exit(1)

    # Verify distributed is initialized
    if not dist.is_initialized():
        _log_system_error("Distributed training not properly initialized")
        sys.exit(1)
    ...
    ), f"draft_global_batch_size={args.draft_global_batch_size}\
        must be divisible by dp_size={args.dp_size}\
        and micro_batch_size={args.draft_micro_batch_size}"
    print_with_rank(
                f"draft_accumulation_steps=\
                {args.draft_global_batch_size} // {args.dp_size} // \
                {args.draft_micro_batch_size}={args.draft_accumulation_steps}"
    )
    ...
    if args.report_to == "azure_ml":
        ...
        # For other tracker types,
        # use the get_tracker_class function from specforge
    ...
    if args.resume:
        ...
        if os.path.isdir(args.output_dir):
            logger.info(f"Checking for checkpoints in output directory: {args.output_dir}")
            draft_model_last_checkpoint = get_last_checkpoint(args.output_dir)

            if draft_model_last_checkpoint:
                logger.info(f"Found checkpoint in output directory: {draft_model_last_checkpoint}")
                ...
                if os.path.isdir(args.resume_from_checkpoint) \
                 and os.path.exists(os.path.join(args.resume_from_checkpoint, CONFIG_JSON)):
                    ...
                    logger.info(f"No checkpoint in output directory.\
                ...
                # No checkpoint in output folder and no user input - soft set resume to False
                warning_msg = f"WARNING: resume=True but no checkpoint found\
                        in output directory ({args.output_dir})\
                        and no resume_from_checkpoint provided.\
                        Setting resume=False and starting training from scratch."
            ...
            # No output directory and no user input - soft set resume to False
            warning_msg = f"WARNING: resume=True but output directory does not exist ({args.output_dir})\
                and no resume_from_checkpoint provided. Setting resume=False and starting training from scratch."
            _log_user_error(warning_msg)
            sys.exit(1)
    else:
        ...
        if args.resume_from_checkpoint:
            if os.path.isdir(args.resume_from_checkpoint)\
                ...
                _log_user_error(f"ERROR: Invalid resume_from_checkpoint path provided: {args.resume_from_checkpoint}")
                sys.exit(1)
    ...
    if args.tp_size > 1:
        ...
        # For future VLM support not exposed to users yet
    ...
    try:
        ...
        _log_system_error(f"ERROR: Failed to load training dataset: {e}")
        sys.exit(1)

    with rank_0_priority():
        print_on_rank0("Building Draft-Model dataset...")
        print_on_rank0(r"Dataset build parameters:")
        ...
        try:
            train_draft_dataset = build_eagle3_dataset(
            ...
            print_on_rank0(f"Successfully built draft model dataset with\
                {len(train_draft_dataset)} examples")
        except Exception as e:
            _log_system_error(f"ERROR: Failed to build draft model dataset: {e}")
            sys.exit(1)

        vocab_mapping_path = generate_vocab_mapping_file(
            dataset=train_draft_dataset,
    ...
    # Calculate total steps if set to -1, 0, or None (auto-calculate)
    if args.total_steps is None or args.total_steps <= 0:
        ...
        print_with_rank(
            f"Auto-calculated total_steps: {args.total_steps}\
                (num_epochs={args.num_epochs} * steps_per_epoch={steps_per_epoch})"
        )
        logger.info(f"Auto-calculated total_steps: {args.total_steps}")
    else:
        print_with_rank(f"Using user-provided total_steps: {args.total_steps}")
        logger.info(f"Using user-provided total_steps: {args.total_steps}")
        # Calculate effective num_epochs based on total_steps
        steps_per_epoch = math.ceil(
            len(train_dataloader) / args.draft_accumulation_steps
        )
        effective_epochs = math.ceil(args.total_steps / steps_per_epoch)
        if effective_epochs != args.num_epochs:
            print_with_rank(
                f"total_steps={args.total_steps} overrides num_epochs={args.num_epochs}. "
                f"Effective epochs: {effective_epochs}"
            )
            print_with_rank(
                f"WARNING: total_steps={args.total_steps} will result in ~{effective_epochs} epochs "
                f"(overriding num_epochs={args.num_epochs})"
            )
    ...
    # refer only else till VLM support is open
    if args.is_vlm and getattr(draft_model_config, "target_model_type", None) == "qwen2_5_vl":
    ...
    print_with_rank("Initialized Draft FSDP model")
    ...
    if args.resume and draft_model_last_checkpoint is not None:
        ...
        if os.path.exists(state_path):
            ...
            print_on_rank0(
                f"Warning: Checkpoint directory {draft_model_last_checkpoint}\
                    found, but training_state.pt is missing. Starting from scratch."
            )
        ...
        print_on_rank0(f"Draft model initialized from checkpoint {draft_model_last_checkpoint},\
            but starting training from epoch 0 (resume=False)")
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for data in progress_bar:
            batch_index += 1
            # VLM is not exposed to users yet
            if args.is_vlm:
                # For VLM, QwenVLOnlineEagle3Model handles target model internally
                ...
                # For non-VLM, OnlineEagle3Model expects pre-computed hidden_states and target
                with torch.no_grad():
                    ...
                    # Extract auxiliary hidden states from 3 layers
        ...
        if args.dataset_validation_split is not None and epoch % args.eval_interval == 0:
            ...
            for data in tqdm(eval_dataloader, desc=f"Evaluating Epoch {epoch}"):
                if args.is_vlm:
                    ...
                    with torch.no_grad():
                        # Extract hidden states and target from target model
                        outputs = target_model(
                            input_ids=data["input_ids"].cuda(),
                            attention_mask=data["attention_mask"].cuda(),
                            output_hidden_states=True,
