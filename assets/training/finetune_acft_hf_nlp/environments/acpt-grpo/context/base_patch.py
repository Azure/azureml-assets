...
class DeviceCommunicatorBase:
    ...
    def all_gather(self, input_: torch.Tensor, dim: int = -1) -> torch.Tensor:
        ...
        # Check and handle dtype mismatch
        if input_.dtype != output_tensor.dtype:
            print(f"Warning: input dtype ({input_.dtype}) != output dtype ({output_tensor.dtype})")

        # If either tensor is bfloat16, convert both to bfloat16
        if input_.dtype == torch.bfloat16 or output_tensor.dtype == torch.bfloat16:
