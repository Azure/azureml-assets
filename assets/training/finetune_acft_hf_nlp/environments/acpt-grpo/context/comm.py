...
def all_gather_into_tensor(output_tensor,
    ...
    global cdb
    input_tensor=tensor
    preferred_dtype = torch.bfloat16 if (
        input_tensor.dtype == torch.bfloat16 or output_tensor.dtype == torch.bfloat16
    ) else output_tensor.dtype

    # Convert both tensors to the preferred dtype
    if input_tensor.dtype != preferred_dtype:
        input_tensor = input_tensor.to(preferred_dtype)
    if output_tensor.dtype != preferred_dtype:
        output_tensor = output_tensor.to(preferred_dtype)
