...
"""Patch Ray's vendored Apache HttpCore classes in its distribution jar."""
...
ARTIFACT = "httpcore5"
MAVEN_GROUP = "org.apache.httpcomponents.core5"
MAVEN_PREFIX = f"META-INF/maven/{MAVEN_GROUP}/{ARTIFACT}/"
CLASS_PREFIX = "org/apache/hc/core5/"
HTTP2_CLASS_PREFIX = "org/apache/hc/core5/http2/"
HTTP2_VERSIONED_FRAGMENT = "/org/apache/hc/core5/http2/"
...
def download_maven_artifacts(version: str) -> tuple[bytes, bytes]:
    """Download the HttpCore jar and pom artifacts."""
...
def is_httpcore5_entry(filename: str) -> bool:
    """Return whether a jar entry belongs to the selected HttpCore artifact."""
    return filename.startswith(CLASS_PREFIX) or (
        filename.startswith("META-INF/versions/")
        and f"/{CLASS_PREFIX}" in filename
    ) or filename.startswith(MAVEN_PREFIX)


def is_separate_http2_entry(filename: str) -> bool:
    """Preserve entries owned by the separate httpcore5-h2 artifact if present."""
    return filename.startswith(HTTP2_CLASS_PREFIX) or HTTP2_VERSIONED_FRAGMENT in filename
...
def build_replacement_entries(
    ...
    """Build replacement HttpCore entries from downloaded artifacts."""
    replacements = {}
    ...
    pom_name = f"{MAVEN_PREFIX}pom.xml"
...
def should_remove_old_entry(filename: str, replacements: set[str]) -> bool:
    """Return whether an old HttpCore entry should be removed from the fat jar."""
    if filename in replacements:
        return True
    if is_separate_http2_entry(filename):
        return False
    return is_httpcore5_entry(filename)
...
def patch_ray_dist(
    ...
    """Patch Ray's distribution jar with replacement HttpCore entries."""
    ...
    try:
        with zipfile.ZipFile(ray_dist, "r") as source, zipfile.ZipFile(
            ...
            for source_info in source.infolist():
                ...
                elif should_remove_old_entry(source_info.filename, replacement_names):
...
def validate_patch(ray_dist: pathlib.Path, version: str) -> None:
    """Validate that the patched jar reports the expected HttpCore version."""
    properties_name = f"{MAVEN_PREFIX}pom.properties"
...
def main() -> None:
    """Patch Ray's vendored HttpCore dependency."""
