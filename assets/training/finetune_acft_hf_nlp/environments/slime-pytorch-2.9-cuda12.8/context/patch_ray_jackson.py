...
"""Patch Ray's vendored Jackson artifacts in its distribution jar."""
...
def download_maven_artifacts(artifact: dict[str, object], version: str) -> tuple[bytes, bytes]:
    ...
    base_url = f"https://repo.maven.apache.org/maven2/com/fasterxml/jackson/core/{name}/{version}"
...
def maven_prefix(artifact: dict[str, object]) -> str:
    """Return the Maven metadata prefix for an artifact."""
    return f"META-INF/maven/com.fasterxml.jackson.core/{artifact['name']}/"


def is_artifact_entry(filename: str, artifact: dict[str, object]) -> bool:
    """Return whether a jar entry belongs to the Maven artifact."""
    prefixes = tuple(str(prefix) for prefix in artifact["entry_prefixes"])
    maven_metadata = maven_prefix(artifact)
    return filename.startswith(prefixes) or filename.startswith(maven_metadata)
...
def build_replacement_entries(
    ...
    pom_name = f"{maven_prefix(artifact)}pom.xml"
...
def validate_patch(ray_dist: pathlib.Path, artifact: dict[str, object], version: str) -> None:
    ...
    properties_name = f"{maven_prefix(artifact)}pom.properties"
...
def main() -> None:
    """Patch Ray's vendored Jackson dependencies."""
