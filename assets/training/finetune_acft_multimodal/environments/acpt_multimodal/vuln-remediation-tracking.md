...
Image: `public/azureml/curated/acft-multimodal-gpu:78`
...
| 5014615 / GHSA-vgrw-7cvw-pwgx | `torch` | Inherited from the ACPT base image in `/opt/conda/envs/ptca`; no patched `torch280` base line is available, so this image overrides the inherited CUDA PyTorch stack. | No | `context/Dockerfile` | `torch==2.10.0+cu126` from the CUDA 12.6 PyTorch index |
| 5014620 / GHSA-qfhq-4f3w-5fph | `torch` | Inherited from the ACPT base image in `/opt/conda/envs/ptca`; no patched `torch280` base line is available, so this image overrides the inherited CUDA PyTorch stack. | No | `context/Dockerfile` | `torch==2.10.0+cu126` from the CUDA 12.6 PyTorch index |
...
| `setuptools>=82.0.1`, `nltk>=3.9.4`, `transformers>=5.0.0`, `onnx>=1.21.0`, `pyOpenSSL>=26.0.0`, `urllib3>=2.7.0` | Kept; these run after this image's package installs because direct and transitive parents still expose loose vulnerable ranges or resolve at build time. |
| `python-dotenv>=1.2.2`, `idna>=3.15`, `click>=8.3.3`, `aiohttp>=3.14.0`, `PyJWT>=2.13.0`, `py-rattler>=0.24.0` | Kept; base SBOM is patched for several of these, but this image installs AzureML SDK/inference dependencies after the base layer and existing comments document the transitive CVE remediation. |
