...
class ReplicaManager:
    ...
    def __init__(self):
        ...
        self._tensor_parallel = int(
            os.getenv("AML_TENSOR_PARALLEL_SIZE", torch.cuda.device_count()))
    ...
    def initialize(self):
        ...
        flag_file_path = os.path.join(
            tempfile.gettempdir(), "model_loaded_flag.txt")
...
Using tensor parallel of {self._tensor_parallel} GPUs per replica.",
    ...
    def _launch_single_replica(self, replica_idx):
        ...
        replica_gpu_ids = self._get_gpu_ids_for_replica(
            replica_idx, gpu_ids_list)
        cuda_visible_devices = ",".join(map(str, replica_gpu_ids))
        logger.debug(
            f"Setting CUDA_VISIBLE_DEVICES to {cuda_visible_devices} for replica {replica_idx + 1}")
        ...
        try:
            ...
            engine_replica.wait_until_server_healthy(CommonConstants.HOST, os.getenv(
                EnvironmentVariables.ENGINE_STARTUP_PORT, str(CommonConstants.DEFAULT_PORT)))
        except Exception as e:
            logger.error(
                f"Failed to start replica {replica_idx + 1} with GPUs {cuda_visible_devices}: {e}")
            traceback.print_exc()
