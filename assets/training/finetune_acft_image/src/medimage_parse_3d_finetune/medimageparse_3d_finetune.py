...
from azureml.acft.image.components.olympus_biomed_parse_3d.checkpoint_loaders.safetensors_loader import (
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description="Execute MedImageParse 3D fine-tuning")
...
def execute_training(args):
    ...
    os.environ["AMLT_EXPERIMENT_NAME"] = "MIP3D_finetune"
    os.environ["AMLT_JOB_NAME"] = "MIP3D_finetune_job"
    ...
    if RANK == 0:
        ...
        # If pretrained model is an MLflow directory, copy catalog files (skip weights).
        # If it's just a raw checkpoint in a blob container, skip this step.
        mlmodel_file = Path(args.pretrained_mlflow_model) / "MLmodel"
        if mlmodel_file.exists():
            logger.info("Found MLflow model structure, copying catalog...")
            copy_catalog_except_weights(args.pretrained_mlflow_model, args.mlflow_model_folder)
        else:
            logger.info("No MLflow model structure found (raw checkpoint mode), skipping catalog copy.")
    ...
    olympus_args = [
        ...
        str(
            importlib.resources.files(
                "azureml.acft.image.components.olympus_biomed_parse_3d"
            ) / "configs"
        ),
...
def main():
    """Run the MedImageParse3D fine-tuning script."""
    logger.info("MedImageParse3D Fine-tuning Script")
    logger.info("=" * 40)
