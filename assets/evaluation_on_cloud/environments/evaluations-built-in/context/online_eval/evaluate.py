...
def extract_score(data: Dict[str, Any]) -> Tuple[List[str], float]:
        ...
        if len(numeric_keys) == 1:
            return list(numeric_keys.keys()), list(numeric_keys.values())[0]
        
        # Step 3: Try for keys with '_score' suffix
        score_keys = {k: v for k, v in numeric_keys.items() if k.endswith('_score')}
        
        if len(score_keys) == 1:
            return list(score_keys.keys()), list(score_keys.values())[0]
        ...
        if len(score_keys) == 0:
            ...
            if len(non_gpt_keys) == 1:
                return list(non_gpt_keys.keys()), list(non_gpt_keys.values())[0]
