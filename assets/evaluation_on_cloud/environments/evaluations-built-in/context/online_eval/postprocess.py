...
def log_evaluation_event(row) -> None:
    ...
    if "trace_id" not in row:
        logger.debug(f"Missing trace_id field from the user query result, taking default from column {DEFAULT_TRACE_ID_COLUMN}")
        trace_id = int(row.get("trace_id", row.get(DEFAULT_TRACE_ID_COLUMN, "0")), 16)
    if "span_id" not in row:
        logger.debug(f"Missing span_id field from the user query result, taking default from column {DEFAULT_SPAN_ID_COLUMN}")
