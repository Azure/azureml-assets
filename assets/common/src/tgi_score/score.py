...
def is_server_healthy():
    ...
    while count < RETRY_COUNT and TEXT_GEN_LAUNCHER_PROCESS_NAME not in [p.name() for p in psutil.process_iter()]:
        logger.warning(
            f"Process {TEXT_GEN_LAUNCHER_PROCESS_NAME} is not running. " f"Sleeping for {WAIT_TIME}s and retrying"
        )
        time.sleep(WAIT_TIME)
        count += 1
    if count >= RETRY_COUNT:
        total_dur = RETRY_COUNT * WAIT_TIME
        raise Exception(f"Sever process not running after waiting for {total_dur}. " f"Terminating")

    logger.info(f"Server process {TEXT_GEN_LAUNCHER_PROCESS_NAME} running. " f"Hitting endpoint with 5s delay")
    time.sleep(5)
...
def init():
    ...
    try:
        ...
        if SHARDED not in os.environ and NUM_SHARD not in os.environ:
            ...
            try:
                ...
                if not (is_sm75 or is_sm8x or is_sm90):
                    ...
                    logger.info(f"Setting {NUM_SHARD} to 1 since flash attention " f"can't be used on this GPU.")
        ...
        while not is_server_healthy():
            logger.info(f"Server not up. Waiting for {WAIT_TIME}s, " f"before querying again.")
            time.sleep(WAIT_TIME)
...
def run(data):
    ...
    try:
        ...
        if severity > aacs_threshold:
            logger.warning(f"Input severity ({severity}) greater than aacs threshold " f"({aacs_threshold}).")
            return {}
        ...
        logger.info(f"generating response for input_string: {query}, " f"parameters: {params}")
