...
class Configuration(Namespace):
    ...
    def is_embeddings(self) -> bool:
        ...
        return self.api_type in [ApiType.Embedding, "embeddings"]

    def is_chat_completion(self) -> bool:
        """Check if the target endpoint is for chat completion models."""
        return (
            self.request_path == constants.OSS_CHAT_COMPLETIONS_API_PATH
            or (
                self.scoring_url is not None
                and (
                    self.scoring_url.endswith(constants.OSS_CHAT_COMPLETIONS_API_PATH)
    ...
    def is_completion(self) -> bool:
        ...
        return (
            self.request_path == constants.OSS_COMPLETIONS_API_PATH
            or (
                self.scoring_url is not None
                and (
                    self.scoring_url.endswith(constants.OSS_COMPLETIONS_API_PATH)
