...
if ! az ml workspace show -n $workspace --output none >/dev/null 2>&1; then
    ...
    workspace_container_registry_id=$(az ml workspace show -n $workspace --query "container_registry" -o tsv | tr -d '\n\r')
    if [ -n "$workspace_container_registry_id" ] && [ "$workspace_container_registry_id" != "None" ]; then
        echo "Checking workspace container registry ${workspace_container_registry_id}"
        if ! az acr show --ids "$workspace_container_registry_id" --output none >/dev/null 2>&1; then
            workspace_container_registry="${workspace_container_registry_id##*/}"
            workspace_container_registry_group=$(echo "$workspace_container_registry_id" | sed -n 's#.*[Rr]esource[Gg]roups/\([^/]*\)/providers/Microsoft.ContainerRegistry/registries/.*#\1#p')
            if [ -z "$workspace_container_registry_group" ]; then
                echo "Unable to parse resource group from workspace container registry ID ${workspace_container_registry_id}"
                exit 1
            fi
            echo "Creating workspace container registry ${workspace_container_registry}"
            az acr create -g "$workspace_container_registry_group" -n "$workspace_container_registry" --sku Basic --admin-enabled
        fi
    fi
fi
