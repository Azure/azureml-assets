...
def create_acr_task(image_name: str,
    ...
    if trivy_url is not None:
        if os is assets.Os.LINUX:
            task['steps'].append({
                ...
                'cmd': f"mcr.microsoft.com/oss/v2/aquasecurity/trivy:{TRIVY_VERSION} -q --timeout {TRIVY_TIMEOUT} image --scanners vuln --ignore-unfixed $Registry/{image_name}",
                'ignoreErrors': True,
                'privileged': True
            })
        else:
    ...
    if push:
        ...
        task['steps'].append({
            ...
            'cmd': f"mcr.microsoft.com/oss/v2/aquasecurity/trivy:{TRIVY_VERSION} image --no-progress --format spdx-json --output sbom.json $Registry/{image_name} --timeout 15m0s",
        ...
        task['steps'].append({
            ...
            'cmd': f"mcr.microsoft.com/oss/v2/oras-project/oras:{ORAS_VERSION} attach $Registry/{image_name} --artifact-type application/vnd.microsoft.artifact.sbom+json --annotation 'org.opencontainers.image.created={timestamp}' --annotation 'org.opencontainers.image.title=SBOM generated by Trivy' --annotation 'org.opencontainers.image.description=Software Bill of Materials generated by Trivy' sbom.json:application/json",
