...
def retrieve_manifest(request, repo, tag):
    global counter, prev_time
    if counter == 1: prev_time = time.time()
    time_elapsed = time.time() - prev_time
    prev_time = time.time()
    try:
        logger.log_debug(f"Attempting to open url {request.full_url}")
        print("Tag:", tag, "Counter:", counter)
        print("Time elapsed since previous:", time_elapsed)
        if counter <= 3:
            counter += 1
            raise Exception("Exception reached, try " + str(counter))
