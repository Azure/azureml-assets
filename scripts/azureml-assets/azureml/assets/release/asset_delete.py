...
SUPPORTED_ASSET_TYPES = [assets.AssetType.COMPONENT.value, assets.AssetType.ENVIRONMENT.value]
...
def delete_single_version(base_url: str, asset_id: str, token: str):
    ...
    if response.status_code != 202:
        ...
        logger.log_warning(response.text)
...
def delete_all_versions(
    ...
    delete_single_version(base_url, asset_id, token)
    container_id = CONTAINER_ASSET_ID_TEMPLATE.substitute(
        asset_name=asset_name, asset_type=f'{asset_type}s', registry_name=registry)
