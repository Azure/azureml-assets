...
def _parsing_mask(mask_fname):
    ...
    binary_masks = _binarise_mask(mask_fname)
    polygons = []
    for bi_mask in binary_masks:
        if len(bi_mask.shape) == 2:
            bi_mask = bi_mask[np.newaxis, :]
        polygon = _convert_mask_to_polygon(bi_mask)
        ...
        with open(validation_annotations_file, "w") as validation_f:
            for i, filename in enumerate(os.listdir(annotations_folder)):
                if filename.endswith(".xml"):
                    ...
                    polygons = _parsing_mask(mask_fname)
