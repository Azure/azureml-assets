...
"""Tests running a sample job in the pytorch 1.10 environment."""
import os
import time
...
def test_pytorch_1_10():
    """Tests a sample job using pytorch 1.10 as the environment."""
    ...
    env_name = "pytorch1_10"

    env_docker_context = Environment(
        build=BuildContext(path=this_dir / BUILD_CONTEXT),
        name=env_name,
        description="Pytorch 1.10 environment created from a Docker context.",
    )
    ...
    job = command(
        ...
        experiment_name="pytorch110Experiment"
    )
