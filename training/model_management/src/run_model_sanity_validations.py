...
def _load_and_prepare_data(test_data_path: Path, mlmodel: Dict, col_rename_map: Dict) -> pd.DataFrame:
    ...
    if mlmodel.get("signature", None):
        ...
        tc.track_event(name="FM_import_pipeline_debug_logs",
                            properties={"message":f"signature is missing from MLModel file."})
    ...
    if input_signatures_str:
        ...
        tc.track_event(name="FM_import_pipeline_debug_logs", properties={"message":f"input_signatures: {input_signatures}"})
        ...
        tc.track_event(name="FM_import_pipeline_debug_logs", 
                       properties={"message":f"Input signature missing in MLmodel. Prediction might fail."})
...
def _load_and_infer_model(model_dir, data):
    if data is None:
        tc.track_event(name="FM_import_pipeline_debug_logs",
                            properties={"message":f"Data not shared. Could not infer the loaded model"})
        tc.flush()
    ...
    try:
        tc.track_event(name="FM_import_pipeline_debug_logs",
                            properties={"message":f"Predicting model with test data!!!"})
        ...
        tc.track_event(name="FM_import_pipeline_debug_logs",
                            properties={"message":f"Error in predicting model: {e}"})
        tc.flush()
...
if __name__ == "__main__":
    ...
    tc.track_event(name="FM_import_pipeline_debug_logs", properties={"message":f"##### logger.info args #####"})
    ...
    with open(mlmodel_file_path) as f:
        ...
        tc.track_event(name="FM_import_pipeline_debug_logs", properties={"message":f"mlmodel :\n{mlmodel_dict}\n"})
        logger.info(f"mlmodel :\n{mlmodel_dict}\n")

    with open(conda_env_file_path) as f:
        conda_dict = yaml.safe_load(f)
        tc.track_event(name="FM_import_pipeline_debug_logs", properties={"message":f"conda :\n{conda_dict}\n"})
        logger.info(f"conda :\n{conda_dict}\n")
    ...
    if col_rename_map_str:
        ...
        tc.track_event(name="FM_import_pipeline_debug_logs", properties={"message":f"col_rename_map => {col_rename_map}"})
        tc.flush()
    ...
    tc.track_event(name="FM_import_pipeline_debug_logs", properties={"message":f"Model copied to {output_model_path}"})
    tc.flush()
