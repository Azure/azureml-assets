...
def get_column_value_frequency(df: pyspark_sql.DataFrame, categorical_columns: list, total_count: int):
    """Calculate frequency of value in a column."""
    entries = []
    for column in categorical_columns:
        new_df = df.groupBy(column) \
            .count() \
...
def _jensen_shannon_categorical(baseline_df: pyspark_sql.DataFrame, production_df: pyspark_sql.DataFrame, baseline_count: int, categorical_columns: list):
    """Jensen Shannon computation for categorical column."""
    reference_distribution = get_column_value_frequency(baseline_df, categorical_columns, baseline_count)

    output_df = (DistributionBalanceMeasure()
...
def _psi_categorical(baseline_df: pyspark_sql.DataFrame, production_df: pyspark_sql.DataFrame, baseline_count: int, production_count: int, categorical_columns: list):
    """PSI calculation for categorical columns."""
    # unique values observed in baseline and production dataset
    psi_rows = []
    for column in categorical_columns:
        print("psi_column: ", column)
        baseline_prod_unique_values = production_df.select(column).union(baseline_df.select(column)).distinct()
        all_unique_values = baseline_prod_unique_values.rdd.flatMap(lambda x: x).collect()
        all_unique_values_dict = {str(value): 0 for value in all_unique_values}

        count_baseline_df = baseline_df.groupBy(column) \
            .count()
        count_baseline_df_dict = {str(row[column]): row["count"] for row in count_baseline_df.collect()}
        ...
        count_prod_df = production_df.groupBy(column) \
            .count()
        count_prod_df_dict = {str(row[column]): row["count"] for row in count_prod_df.collect()}
        ...
        for value in production_frequencies:
            ...
            production_ratios[value] = (production_frequencies[value]+1)/production_count

        psi = 0
        for i in baseline_ratios:
            psi += (production_ratios[i] - baseline_ratios[i]) * np.log(production_ratios[i] / baseline_ratios[i])
...
def _chisquaretest(baseline_df, production_df, baseline_count, production_count, categorical_columns) -> float:
    ...
    for column in categorical_columns:
        ...
        production_frequencies_map = {**initial_unique_values_dict, **count_prod_df_dict}
        ...
        chisqtest_row = [column, float(chisqtest_val), 'categorical', 'PearsonsChiSquaredTest']
...
def compute_categorical_data_drift_measures_tests(baseline_df: pyspark_sql.DataFrame, production_df: pyspark_sql.DataFrame, baseline_count: int, production_count: int, categorical_metric: str, categorical_columns: list, categorical_threshold: float):
    """Compute Data drift metrics and tests for numerical columns."""
    if categorical_metric == "JensenShannonDistance":
        output_df = _jensen_shannon_categorical(baseline_df, production_df, baseline_count, categorical_columns)
    elif categorical_metric == "PopulationStabilityIndex":
        output_df = _psi_categorical(baseline_df, production_df, baseline_count, production_count, categorical_columns)
    elif categorical_metric == "PearsonsChiSquaredTest":
        output_df = _chisquaretest(baseline_df, production_df, baseline_count, production_count, categorical_columns)
    else:
        raise Exception(f"Invalid metric {categorical_metric} for categorical feature")

    output_df = output_df.withColumn("threshold_value", F.lit(categorical_threshold).cast("float"))
    return output_df
