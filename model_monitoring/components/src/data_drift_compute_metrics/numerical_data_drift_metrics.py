...
from shared_utilities.histogram_utils import get_dual_histogram_bin_edges, get_histograms


def _map_production_df_columns_with_bins(production_df: pyspark_sql.DataFrame, production_df_count_freq_map: dict):
    ...
    for column in production_df.columns:
        transformed_df = transformed_df.withColumn(column, map_value_to_bucket(broadcasted_prod_df_map, column)(F.col(column)))

    return transformed_df


def _calculate_column_value_frequency_bin_map_in_df(hist, numerical_columns, total_count):
    ...
    return {"feature_bucket_count_map": feature_bucket_count_map, "feature_bucket_frequency_map": feature_bucket_frequency_map, "feature_bucket_edges_map": feature_bucket_edges_map}


def _jensen_shannon_numerical(baseline_df: pyspark_sql.DataFrame, production_df: pyspark_sql.DataFrame, baseline_df_count, production_df_count, numerical_columns: list):
    """Calculate Jensen Shannon metric for numerical columns."""
    bin_edges = get_dual_histogram_bin_edges(baseline_df, production_df, baseline_df_count, production_df_count, numerical_columns)
    ...
    baseline_freq_count_map = _calculate_column_value_frequency_bin_map_in_df(baseline_histograms, numerical_columns, baseline_df_count)
    production_freq_count_map = _calculate_column_value_frequency_bin_map_in_df(prod_histograms, numerical_columns, production_df_count)

    reference_distribution = list(baseline_freq_count_map["feature_bucket_frequency_map"].values())

    transformed_production_df = _map_production_df_columns_with_bins(production_df, production_freq_count_map)
    distributionBalanceMeasure = (DistributionBalanceMeasure()
    ...
    output_df = distributionBalanceMeasure.select("FeatureName", "DistributionBalanceMeasure.js_dist")
    output_df = output_df \
        .withColumnRenamed("FeatureName", "feature_name") \
...
def _normalized_wasserstein(baseline_df: pyspark_sql.DataFrame, production_df: pyspark_sql.DataFrame, baseline_df_count: int, production_df_count: int, numerical_columns: list):
    ...
    distribution_balance_measures = (DistributionBalanceMeasure()
                                     .setSensitiveCols(numerical_columns)
                                     .setReferenceDistribution(reference_distribution)
                                     .transform(transformed_production_df.select(numerical_columns)))

    output_df = distribution_balance_measures.select("FeatureName", "DistributionBalanceMeasure.wasserstein_dist")
    ...
    baseline_df_stdev = baseline_df_stats.where(F.col("summary") == "stddev").drop("summary").first()
    ...
    output_df = output_df.withColumn("metric_value", normalize_value(mapping)(F.col('metric_value'), F.col('feature_name')))
    return output_df


def _ks2sample_pandas_impl(baseline_df, production_df, baseline_count, production_count, numerical_columns):
    ...
    if baseline_count < DATASET_SIZE and production_count < DATASET_SIZE:
        ...
        raise Exception(f"Cannot calculate two-sample Kolmogorov-Smirnov test on dataset with more than {DATASET_SIZE} entries")


def _psi_numerical(baseline_df, production_df, baseline_df_count, production_df_count, numerical_columns):
    ...
    for column in numerical_columns:
        ...
        baseline_percent = [(count/baseline_df_count) for count in baseline_hist_count]
        production_percent = [(count/production_df_count) for count in production_hist_count]

        psi = 0.0
        for i in range(len(baseline_percent)):
            psi += (production_percent[i] - baseline_percent[i]) * np.log(production_percent[i] / baseline_percent[i])
...
def compute_numerical_data_drift_measures_tests(baseline_df: pyspark_sql.DataFrame, production_df: pyspark_sql.DataFrame, baseline_df_count: int, production_df_count: int,  numerical_metric: str, numerical_columns: list, numerical_threshold: float):
    """Compute Data drift metrics and tests for numerical columns."""
    if numerical_metric == "JensenShannonDistance":
        output_df = _jensen_shannon_numerical(baseline_df, production_df, baseline_df_count, production_df_count, numerical_columns)
    elif numerical_metric == "NormalizedWassersteinDistance":
        output_df = _normalized_wasserstein(baseline_df, production_df, baseline_df_count, production_df_count, numerical_columns)
    elif numerical_metric == "PopulationStabilityIndex":
        output_df = _psi_numerical(baseline_df, production_df, baseline_df_count, production_df_count, numerical_columns)
    elif numerical_metric == "TwoSampleKolmogorovSmirnovTest":
        output_df = _ks2sample_pandas_impl(baseline_df, production_df, numerical_columns)
    else:
        raise Exception(f"Invalid metric {numerical_metric} for numerical feature")

    output_df = output_df.withColumn("threshold_value", F.lit(numerical_threshold).cast("float"))
    return output_df
