...
from compute_data_quality_metrics import (
    compute_data_quality_metrics
)


def _filter_metrics(df, metrics, data_type):
    return df.filter((col("metricName").isin(metrics) & (col("dataType") == data_type)) | (col("dataType") != data_type))
...
def run():
    ...
    data_stats_table = data_stats_table.withColumn('set', split(trim('set'), '( +)?, ?'))

    violation_df = compute_data_quality_metrics(df, data_stats_table)
    violation_df = _filter_metrics(violation_df, args.categorical_metrics.split(","), data_type="Categorical")
    violation_df = _filter_metrics(violation_df, args.numerical_metrics.split(","), data_type="Numerical")
